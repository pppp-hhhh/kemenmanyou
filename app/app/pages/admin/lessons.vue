<template>
  <div class="max-w-6xl mx-auto">
    <div class="flex items-center gap-4 mb-6">
      ...
      <h1 class="text-2xl font-bold text-neutral-700 dark:text-neutral-100">课文管理</h1>
    </div>
    ...
    <div class="bg-white dark:bg-surface-800 rounded-lg shadow p-4 mb-6">
      <div class="flex flex-wrap items-center gap-4">
        <input
          ...
          type="text"
          placeholder="搜索课文标题..."
          class="flex-1 min-w-[200px] px-4 py-2 rounded-lg border border-surface-200 dark:border-neutral-700 bg-white dark:bg-surface-900 text-neutral-700 dark:text-neutral-100"
        />
    ...
    <div v-else-if="filteredLessons.length === 0" class="text-center py-12 bg-white dark:bg-surface-800 rounded-lg">
      <p class="text-neutral-500">暂无课文</p>
    </div>
      ...
      <table class="w-full">
        <thead class="bg-surface-50 dark:bg-surface-900">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">标题</th>
            <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">年级</th>
            <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">来源</th>
            <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">创建时间</th>
        ...
        <tbody class="divide-y divide-surface-200 dark:divide-neutral-700">
          <tr v-for="lesson in filteredLessons" :key="lesson.id">
            ...
            <td class="px-6 py-4">
              <div class="text-sm font-medium text-neutral-700 dark:text-neutral-100">{{ lesson.title }}</div>
              <div class="text-sm text-neutral-500 dark:text-neutral-400 truncate max-w-xs">{{ lesson.content?.slice(0, 50) }}...</div>
            </td>
